...
export async function POST(request: NextRequest) {
  try {
    ...
    const incog26Flag = incog26 === true;
    ...
    // Track analytics and get user ID - MUST happen BEFORE processing the message (skip for incognito and incog26)
    let userId: string | null = null;
    if (!incognitoFlag && !incog26Flag && sessionId) {
      try {
      ...
      console.log("Analytics skipped - incognito:", incognitoFlag, "incog26:", incog26Flag, "sessionId:", sessionId);
    }
    ...
    if (model === "openai") {
      ...
      // Filter out system messages if incognito or incog26 (for privacy)
      const filteredMessages = (incognitoFlag || incog26Flag)
    ...
    if (model === "gemini") {
      ...
      try {
        ...
        if (codingMode && !incognitoFlag && !incog26Flag) {
          contents.push({
            role: "user",
    ...
    if (model === "claude") {
      ...
      // Filter out system messages if incognito or incog26
      const filteredMessages = (incognitoFlag || incog26Flag)
        ? messages.filter((msg: any) => msg.role !== "system")
        : messages;
