...
import { Send, Image as ImageIcon, Settings, EyeOff, Command, ThumbsUp, Home, User, Copy, Check, Sparkles, Lock } from "lucide-react";
...
export default function ChatPage() {
  ...
  const [incog26Mode, setIncog26Mode] = useState(() => {
    if (typeof window === "undefined") return false;
    try {
      return localStorage.getItem("trexai_incog26") === "true";
    } catch {
      return false;
    }
  });
  ...
  // Persist incog26 toggle
  useEffect(() => {
    try {
      if (typeof window !== "undefined") {
        localStorage.setItem("trexai_incog26", incog26Mode ? "true" : "false");
      }
    } catch {
      // ignore
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Save chat history (if not incognito and not incog26 — incog26 saves nowhere)
        if (!incognitoMode && !incog26Mode) {
          if (user) {
  ...
  return (
    <div className={`flex flex-col min-h-screen h-screen max-w-[100vw] overflow-x-hidden ${themeClasses.text}`} style={backgroundStyle}>
      ...
      <SettingsPanel
        ...
        incog26Mode={incog26Mode}
        onIncog26Change={setIncog26Mode}
        user={user}
