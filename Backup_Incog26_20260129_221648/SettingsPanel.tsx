...
interface SettingsPanelProps {
  ...
  incog26Mode: boolean;
  onIncog26Change: (value: boolean) => void;
...
export default function SettingsPanel({
  ...
  const handleCodeSubmit = async () => {
    ...
    if (code === "incog26") {
      try {
        localStorage.setItem("trexai_incog26_unlocked", "true");
      } catch {
        // ignore
      }
      onIncog26Change(true);
      setCodeSuccess("✓ Incog26 mode unlocked! Toggle it below. Nothing is saved (not even in Creator history).");
      setCodeError("");
      setCodeInput("");
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className={`${theme === "dark" ? "bg-gray-800 text-white" : "bg-white text-gray-900"} rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4`}>
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Incog26 Mode - no save anywhere, not even in Creator user history */}
          {typeof window !== "undefined" && localStorage.getItem("trexai_incog26_unlocked") === "true" && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="gap-3 flex flex-col sm:flex-row sm:items-center">
                  <div className="flex items-center gap-2">
                    <Lock size={20} className="text-amber-400" />
                    <span className="font-semibold">Incog26 Mode</span>
                  </div>
                  <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                    Nothing is saved: not locally, not in the database, not in Creator Controls user history.
                  </p>
                </div>
                <button
                  onClick={() => onIncog26Change(!incog26Mode)}
                  className={`relative w-14 h-7 rounded-full transition-colors flex-shrink-0 ${incog26Mode ? "bg-amber-600" : "bg-gray-600"}`}
                >
                  <div
                    className={`absolute top-1 left-1 w-5 h-5 bg-white rounded-full transition-transform ${incog26Mode ? "translate-x-7" : "translate-x-0"}`}
                  />
                </button>
              </div>
              {incog26Mode && (
                <div className={`p-3 rounded-lg border ${theme === "dark" ? "bg-amber-900/20 border-amber-700/50 text-amber-200" : "bg-amber-50 border-amber-200 text-amber-800"}`}>
                  <p className={`text-xs font-medium ${theme === "dark" ? "text-amber-300" : "text-amber-700"}`}>
                    🔒 Incog26: Chats are not saved anywhere. Creator cannot see them in user history.
                  </p>
                </div>
              )}
            </div>
